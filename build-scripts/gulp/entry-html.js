...
const renderDemoTemplate = (pth, data = {}) =>
    renderTemplate(pth, data,
                   (tpl) => path.resolve(config.demo_dir, "src/html/",
                                         `${tpl}.html.template`));

const renderCastTemplate = (pth, data = {}) =>
    renderTemplate(pth, data,
                   (tpl) => path.resolve(config.cast_dir, "src/html/",
                                         `${tpl}.html.template`));

const renderGalleryTemplate = (pth, data = {}) =>
    renderTemplate(pth, data,
                   (tpl) => path.resolve(config.gallery_dir, "src/html/",
                                         `${tpl}.html.template`));

const minifyHtml = (content) => minify(content, {
...
gulp.task("gen-pages-prod", (done) => {
  ...
  for (const page of PAGES) {
    ...
    fs.outputFileSync(path.resolve(config.root, `${page}.html`),
                      minifyHtml(content));
  }
  done();
});
...
gulp.task("gen-index-cast-dev", (done) => {
  ...
  fs.outputFileSync(path.resolve(config.cast_root, "receiver.html"),
                    contentReceiver);
  ...
  fs.outputFileSync(path.resolve(config.cast_root, "index.html"),
                    contentLauncher);
  done();
});
...
gulp.task("gen-index-cast-prod", (done) => {
  const latestManifest =
      require(path.resolve(config.cast_output, "manifest.json"));
  const es5Manifest =
      require(path.resolve(config.cast_output_es5, "manifest.json"));
...
gulp.task("gen-index-demo-prod", (done) => {
  const latestManifest =
      require(path.resolve(config.demo_output, "manifest.json"));
  const es5Manifest =
      require(path.resolve(config.demo_output_es5, "manifest.json"));
  const content = renderDemoTemplate("index", {
...
gulp.task("gen-index-gallery-prod", (done) => {
  const latestManifest =
      require(path.resolve(config.gallery_output, "manifest.json"));
  const content = renderGalleryTemplate("index", {
