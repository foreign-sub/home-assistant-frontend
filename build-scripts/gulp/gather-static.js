...
function copyTranslations(staticDir) {
  ...
  // Translation output
  fs.copySync(polyPath("build-translations/output"),
...
function copyPolyfills(staticDir) {
  ...
  copyFileDir(npmPath("@webcomponents/webcomponentsjs/webcomponents-bundle.js"),
              staticPath("polyfills/"));
  copyFileDir(
...
function copyFonts(staticDir) {
  const staticPath = genStaticPath(staticDir);
  // Local fonts
  cpx.copySync(npmPath("roboto-fontface/fonts/roboto/*.woff2"),
               staticPath("fonts/roboto"));
}

function copyMapPanel(staticDir) {
  const staticPath = genStaticPath(staticDir);
  copyFileDir(npmPath("leaflet/dist/leaflet.css"),
              staticPath("images/leaflet/"));
  fs.copySync(npmPath("leaflet/dist/images"),
...
gulp.task("copy-static", (done) => {
  ...
  // Panel assets
  copyFileDir(npmPath("react-big-calendar/lib/css/react-big-calendar.css"),
...
gulp.task("copy-static-demo", (done) => {
  ...
  fs.copySync(polyPath("public/static"),
