...
gulp.task("develop-app",
          gulp.series(
              async function setEnv() { process.env.NODE_ENV = "development"; },
              "clean",
              gulp.parallel("gen-service-worker-dev",
                            gulp.parallel("gen-icons-app", "gen-icons-mdi"),
                            "gen-pages-dev", "gen-index-app-dev",
                            "build-translations"),
              "copy-static", "webpack-watch-app"));
...
gulp.task(
    ...
    gulp.series(
        ...
        "copy-static", "webpack-prod-app",
        ... // Don't compress running tests
        (envVars.isTravis() ? [] : [ "compress-app" ]),
        gulp.parallel("gen-pages-prod", "gen-index-app-prod",
