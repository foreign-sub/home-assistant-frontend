...
let version =
    fs.readFileSync(path.resolve(paths.polymer_dir, "setup.py"), "utf8")
        .match(/\d{8}\.\d+/);
if (!version) {
...
const createWebpackConfig = ({
  ...
  return {
    ...
    plugins : [
      ...
      new webpack.DefinePlugin({
        ...
        "process.env.NODE_ENV" :
            JSON.stringify(isProdBuild ? "production" : "development"),
        ...defineOverlay,
      }),
      // Ignore moment.js locales
      new webpack.IgnorePlugin(/^\.\/locale$/, /moment$/),
      // Color.js is bloated, it contains all color definitions for all material
      // color sets.
    ...
    output : {
      ...
      chunkFilename : isProdBuild && !isStatsBuild ? "chunk.[chunkhash].js"
                                                   : "[name].chunk.js",
...
const createAppConfig = ({isProdBuild, latestBuild, isStatsBuild}) => {
  ...
  if (latestBuild) {
    ...
    const translationMetadata =
        require("../build-translations/translationMetadata.json");
    const workBoxTranslationsTemplatedURLs = {};
    const englishFilename =
        `en-${translationMetadata.translations.en.hash}.json`;

    // core
    workBoxTranslationsTemplatedURLs[`/static/translations/${
        englishFilename}`] = `build-translations/output/${englishFilename}`;

    translationMetadata.fragments.forEach((fragment) => {
      workBoxTranslationsTemplatedURLs[`/static/translations/${fragment}/${
          englishFilename}`] =
