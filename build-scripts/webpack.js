...
const createAppConfig = ({isProdBuild, latestBuild, isStatsBuild}) => {
  ...
  Object.keys(englishFP).forEach((key) => {
    workBoxTranslationsTemplatedURLs[`/static/translations/${englishFP[key]}`] =
  ...
  return {
    ...
    plugins : [
      ...
      ...plugins,
      isProdBuild && !isCI && !isStatsBuild && new CompressionPlugin({
        cache : true,
      ...
      latestBuild && new WorkboxPlugin.InjectManifest({
...
const createDemoConfig = ({isProdBuild, latestBuild, isStatsBuild}) => {
  return {
    ...
    output : {
      filename : genFilename(isProdBuild),
      chunkFilename : genChunkFilename(isProdBuild, isStatsBuild),
      path : path.resolve(paths.demo_root,
...
const createCastConfig = ({isProdBuild, latestBuild}) => {
  ...
  return {
    ...
    output : {
      filename : genFilename(isProdBuild),
      chunkFilename : genChunkFilename(isProdBuild, isStatsBuild),
      path : path.resolve(paths.cast_root,
                          latestBuild ? "frontend_latest" : "frontend_es5"),
      publicPath : latestBuild ? "/frontend_latest/" : "/frontend_es5/",
      // For workerize loader
      globalObject : "self",
