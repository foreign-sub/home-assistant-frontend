...
class HaFormCustomize extends LocalizeMixin
(PolymerElement) {
  static get template() {
    return html`
  ...
  saveEntity() {
    ...
    return this.hass.callApi("POST", "config/customize/config/" + objectId,
                             data);
  }
  ...
  _computeSingleAttribute(key, value, secondary) {
    ...
    return this._initOpenObject(
        key, config.type === "json" ? JSON.stringify(value) : value, secondary,
        config);
  }

  _computeAttributes(config, keys, secondary) {
    return keys.map(
        (key) => this._computeSingleAttribute(key, config[key], secondary));
  }
  ...
  computeGlobalAttributes(localConfig, globalConfig) {
    ...
    const localKeys = Object.keys(localConfig);
    const globalKeys =
        Object.keys(globalConfig).filter((key) => !localKeys.includes(key));
    return this._computeAttributes(globalConfig, globalKeys, true);
  }

  computeExistingAttributes(localConfig, globalConfig, entity) {
    if (!localConfig || !globalConfig || !entity)
      return [];
    const localKeys = Object.keys(localConfig);
  ...
  computeShowWarning(localConfig, globalConfig) {
    ...
    return Object.keys(localConfig)
        .some((key) => JSON.stringify(globalConfig[key]) !==
  ...
  filterFromAttributes(attributes) {
    return (key) => !attributes ||
                    attributes.every((attr) =>
                                         attr.attribute !== key || attr.closed);
  }

  getNewAttributesOptions(localAttributes, globalAttributes, existingAttributes,
                          newAttributes) {
    const knownKeys =
  ...
  selectedNewAttributeObserver(selected) {
    ...
    let result =
        this.localAttributes.findIndex((attr) => attr.attribute === option);
    if (result >= 0) {
    ...
    result =
        this.globalAttributes.findIndex((attr) => attr.attribute === option);
    if (result >= 0) {
    ...
    result =
        this.existingAttributes.findIndex((attr) => attr.attribute === option);
    if (result >= 0) {
  ...
  attributesObserver() {
    ...
    this.newAttributesOptions = this.getNewAttributesOptions(
        this.localAttributes, this.globalAttributes, this.existingAttributes,
        this.newAttributes);
  }
}
