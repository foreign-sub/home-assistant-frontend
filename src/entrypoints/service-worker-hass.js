...
function initRouting() {
  ...
  workbox.routing.registerRoute(new RegExp(`${location.host}/(api|auth)/.*`),
  ...
  // For rest of the files (on Home Assistant domain only) try both cache and
  // network. This includes the root "/" or "/states" response and user files
  // from "/local". First access might bring stale data from cache, but a single
  // refresh will bring updated file.
  workbox.routing.registerRoute(new RegExp(`${location.host}/.*`),
...
function initPushNotifications() {
  ...
  self.addEventListener("push", function(event) {
    ...
    if (event.data) {
      ...
      if (data.dismiss) {
        event.waitUntil(self.registration.getNotifications({tag : data.tag})
                            .then((notifications) =>
  ...
  self.addEventListener("notificationclick", function(event) {
    ...
    if (event.action || !event.notification.data ||
  ...
  self.addEventListener(
      "notificationclose",
      function(event) { notificationEventCallback("closed", event); });
}
